...
async function wordcount():Promise<void>{
    ...
    const paragraph: string = answer.paragraph;

    // Count words (without whitespaces)
    let paragraphList:string[] = paragraph.trim().split(" ");
    let wordCounter:number = paragraphList.length;

    const msg = chalk.rgb(255,128,0)(`Total words are:`,wordCounter);
    const boxedMessage = boxen(msg, 
        { padding: 1, margin: 1, borderStyle: 'double',textAlignment:"center",borderColor:"white" });
