...
let userInfo: { username: string; pin: string };
...
async function authenticateUser(): Promise<{ username: string; pin: string }> {
...
async function display(userInfo: { username: string; pin: string }): Promise<void> {
    const msg = chalk.rgb(255, 153, 51).bold.underline(`Welcome ${userInfo.username}`);
    const boxedMessage = boxen(`${msg}`, 
        { padding: 1, margin: 1, borderStyle: 'double',textAlignment:"center" }
    );
    console.log(boxedMessage);
}

async function withDraw(accountBalance:number):Promise<number>{
...
async function deposit(accountBalance:number):Promise<number>{
...
async function accountInfo(accountBalance: number, userInfo: { username: string; pin: string }):Promise<void>{
...
async function transfer(accountBalance: number): Promise<number> {
...
async function generateReceipt(userInfo: { username: string; pin: string }, remainingBalance: number): Promise<void> {
...
async function promptReceiptGeneration(): Promise<boolean> {
...
async function startAtm(accountBalance:number):Promise<void>{
    const answer = await inquirer.prompt({
        type:'list',
        name:'atm',
...
async function continueOrExit(accountBalance:number): Promise<void> {
    const answer = await inquirer.prompt({
        type: 'list',
        name: 'continueOrexit',
